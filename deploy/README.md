...
- `.github/workflows/deploy.yml`: прежний `cmd/drawing-service`, существующие секреты `VPS_HOST`, `VPS_USER`, `VPS_PORT`, `VPS_SSH_KEY`, `VPS_TARGET`. Версия Go берётся из `go.mod`; тесты выполняются перед сборкой. Binary загружается как `drawing-service.next`, затем атомарно заменяется; старая версия сохраняется как `drawing-service.previous`. Остановленный сервис остаётся остановленным.
...
Загрузка идёт в `/opt/crocodile/incoming/<api|web>-<run>-<attempt>-<sha>`. Скрипт удерживает общий `flock` между двумя репозиториями, копирует текущий релиз, заменяет только нужный компонент и атомарно переключает `current`. API проверяется по ответу 404 на несуществующую комнату, frontend — сравнением HTML через локальный HTTPS vhost. При ошибке возвращается предыдущая ссылка. Фронтенд не требует перезапуска Nginx или API. База `/var/lib/crocodile/game.db` не копируется и не заменяется.

Старые релизы и хешированные assets пока сохраняются для отката и открытых вкладок; автоматическая очистка не включена. Изолированная проверка на Linux: `python3 deploy/test-deploy.py`. Статическая проверка: `shellcheck deploy/deploy-crocodile.sh` и `actionlint`.
...
- Версия: `/opt/crocodile/releases/20260906-1600`; `/opt/crocodile/current` — символьная ссылка на неё. Бинарный файл `crocodile`, фронтенд в `web/`.
...
## Соседние сайты и ранее существовавшие проблемы
...
Для следующей версии сначала собрать `cmd/crocodile` под Linux amd64 и фронтенд `npm run build`, загрузить их в новый каталог `releases`, проверить контрольные суммы. Переключать ссылку `current` атомарным переименованием временной ссылки и перезапускать только `crocodile.service`. Базу не копировать поверх существующей. Для резервной копии работающей bbolt использовать транзакционный snapshot; обычную копию файла делать при остановленном только игровом сервисе.
